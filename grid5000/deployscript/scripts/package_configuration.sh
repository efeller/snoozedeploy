...
configure_libvirt() {
	echo "$log_tag Configuring libvirt"
...
create_client_configuration () {
    local bootstrap_addresses=$(attach_port_to_hosts "bootstrap_nodes.txt" $start_control_data_port)
    sed 's/^general.bootstrapNodes.*/general.bootstrapNodes = '$bootstrap_addresses'/g' "$config_templates_directory/$client_config_name" > "$tmp_directory/$client_config_name" 
}
...
create_bootstrap_configuration () {
    ...
    perl -pi -e "s/^network.multicast.address.*/network.multicast.address = $multicast_address/" "$tmp_directory/snooze_node_bs.cfg"
}
...
create_local_controller_configuration () {
    ...
    perl -pi -e "s/^network.multicast.address.*/network.multicast.address = $multicast_address/" "$tmp_directory/snooze_node_lc.cfg"
}

# Deploys the kapower3 configuration files
deploy_kapower3_configuration () {
    run_taktuk "$tmp_directory/group_managers.txt" put "[ $tmp_directory/$kapower_config_name ] [ /etc/kadeploy3/$kapower_config_name ]"
}
