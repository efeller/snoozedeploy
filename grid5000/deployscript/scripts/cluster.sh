...
list_cluster_addresses() 
{
    ...
    echo "$log_tag Bootstrap node addresses:" $(cat $tmp_directory/bootstrap_nodes.txt)
    echo "$log_tag Group leader/manager addresses:" $(cat $tmp_directory/group_managers.txt)
    echo "$log_tag Local controller addresses:" $(cat $tmp_directory/local_controllers.txt)  
}
